...
- **Only the first 1 MiB of a file is scanned** — a file is read up to a 1 MiB cut rather than to its end, because an unbounded read of a multi-GB log keeps the hook from returning, and a hook killed by the PreToolUse timeout does not block the call. A secret sitting past the cut is missed, the same trade the 64 KB transcript tail read makes.
